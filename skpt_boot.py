...
from src.utils.data.skpt_loader import prepare_data_loader
...
def train(model, train_set, dev_set):
    ...
    try:
        ...
        best_ppl = 1000
        patient = 0
        # different model in different folder
        log_dir = config.save_path + f"/{config.model}"
...
def test(model, test_set):
    ...
    with open(file_summary, "w") as f:
        f.write("EVAL:\tLoss\tPPL\tBce\tAccuracy\n")
        f.write(
            "{}\t{:.4f}\t{:.4f}\t{:.4f}\n".format(
